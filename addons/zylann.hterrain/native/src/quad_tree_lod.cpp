...
Variant QuadTreeLod::_make_chunk(int lod, int origin_x, int origin_y) {
	if (_make_func.is_valid()) {
		Array _make_args;
		_make_args[0] = origin_x;
		_make_args[1] = origin_y;
		_make_args[2] = lod;
		return _make_func.callv(_make_args);
	} else {
		return Variant();
	}
}
...
void QuadTreeLod::_recycle_chunk(unsigned int quad_index, int lod) {
	...
	if (_recycle_func.is_valid()) {
		Array _make_args;
		_make_args[0] = quad->get_data();
		_make_args[1] = quad->origin_x;
		_make_args[2] = quad->origin_y;
		_make_args[3] = lod;
		_recycle_func.callv(_make_args);
	}
}
...
void QuadTreeLod::_update(unsigned int quad_index, int lod, Vector3 view_pos) {
	...
	if (_vertical_bounds_func.is_valid()) {
		Array _make_args;
		_make_args[0] = quad->origin_x;
		_make_args[1] = quad->origin_y;
		_make_args[2] = lod;
		Variant result = _vertical_bounds_func.callv(_make_args);
...
void QuadTreeLod::_bind_methods()
{
	...
	ClassDB::bind_method(D_METHOD("set_split_scale", "p_split_scale"), &QuadTreeLod::set_split_scale);
